    ...
    <table>
            <thead>
                <tr align="center">
                    ...
                    <th width="8%">PC</th>
                    <th width="3%">Store</th> 
                    <th width="4%">Type PC</th>
                    <th width="4%">Sale TV</th>
                    <th width="4%">Sale AV</th>
                    <th width="4%">Sale HA</th>
                    <th width="6%">Sale Total</th>
                    <th width="4%">Target</th>
                    <th width="5%">%Achieve<br>TV+AV</th> 
                    <th width="4%">Normal<br>ComTV+AV</th> 
                    <th width="3%">%<br>ComTV+AV</th> 
                    <th width="3%">%<br>Com HA</th>
                    <th width="3%">Pay<br>Com</th>
                    <th width="3%">Extra</th> 
                    <th width="3%">Other</th> 
                    <th width="3%">Net<br>Com</th>
                    <th width="3%">Advance</th> 
                    <th width="3%">Net Pay</th> 
                    <th width="3%">% Com</th> 
                    <th width="3%">Salary</th> 
                    <th width="4%">{{ $currentMonthName->short_en }}</th> 
                    <th width="4%">{{ $previousMonthName1->short_en }}</th> 
                    <th width="4%">{{ $previousMonthName2->short_en }}</th> 
        ...
        <tbody>
            ...
            <tr>
                ...
                <td align="center">{{ $pcs->code_pc }}</td> 
                ...
                <td align="center">{{ $commission->tarket != 0 ? number_format(($commission->sale_total / $commission->tarket) * 100, 2) : '0' }} %</td> 
                ...
                <td align="center">{{ $commission->remark }}</td> 
            </tr>
            @endforeach
            <tr>
        ...
        <table>
            <thead>
                <tr align="center">
                    <th width="10%">ประเภทพนักงาน</th>
                    <th width="15%">Achieve</th> 
                    <th width="15%">Com</th>
                    <th width="15%">% Com</th>
            ...
            <tr>
                <td>{{ $data->type_pc }}</td>
                <td>{{ number_format($achieve , 0) }}</td>
                <td>{{ number_format($com , 0) }}</td>
                <td>{{ number_format($percentCom , 2) }}%</td>
                <td>{{ number_format($countPc , 0) }}</td>
            </tr>
            @php
            ...
            @endphp
                <tr>
                    <td>Sum Total</td>
                    <td>{{ number_format($sumSaleTotal , 0) }}</td>
                    <td>{{ number_format($sumCom , 0) }}</td>
                    <td>{{ number_format($sumpercentCom , 2) }}%</td>
                    <td>{{ number_format($sumCountPc , 0) }}</td>
                </tr>
            </tbody>
        </table>
        ...
        <table width="40%">
            ...
            <tbody>
                ...
                @endphp 
                <tr>
                    <td>{{ $currentMonthName->short_en }}</td>
                    <td>{{ number_format($query_sale_in_sum->sale_in,0) }}</td>
                    <td>{{ number_format($query_total_sum->total_sale,0) }}</td>
                </tr>
                @php
            ...
            <tr>
                <td>Sum All {{ $year }}</td>
                <td>{{ number_format($query_sale_in_sum_year->sale_in_total, 0) }}</td>
                <td>{{ number_format($query_total_sum_year->total_sale, 0) }}</td>
            </tr>
            </tbody>
        </table>
        ...
        <br><hr>
            <table>
                <thead>
                    <tr align="center">
                        ...
                        <th width="9%">Sale Out</th>
                        <th width="7%">Achieve</th>
                        <th width="7%">Base Com</th>
                        <th width="6%">Com</th>
                        <th width="6%">Extra <br>Sale Out</th>
                        <th width="6%">Extra <br>Unit</th>
                        <th width="6%">Extra <br>AVG</th>
                        <th width="6%">Other</th> 
                        <th width="8%">Total</th>
            ...
            @foreach ($commissions_sale as $commissionSale) 
                ...
                @endphp
            <tr>
                <td align="center">{{ $num_row++; }}</td> 
                <td align="center">{{ $commissionSale->code_sale }}</td> 
                <td align="left">{{ $commissionSale->name_sale }}</td> 
                <td align="center">{{ number_format($commissionSale->target ,0) }}</td> 
                <td align="center">{{ number_format($commissionSale->sale_out ,0) }}</td> 
                <td align="center">{{ number_format($commissionSale->achieve,2) }}%</td> 
                <td align="center">{{ number_format($commissionSale->base_com,0) }}</td> 
                <td align="center">{{ number_format($commissionSale->com_sale,0) }}</td> 
                <td align="center">{{ number_format($commissionSale->extra_sale_out,0) }}</td> 
                <td align="center">{{ number_format($commissionSale->extra_unit,0) }}</td> 
                <td align="center">{{ number_format($commissionSale->extra_avg,0) }}</td> 
                <td align="center">{{ number_format($commissionSale->other,0) }}</td> 
                <td align="center">{{ number_format($commissionSale->total,0) }}</td> 
                <td align="center">{{ $commissionSale->remark }}</td> 
            </tr>
            @endforeach
            </tbody>
            <tfoot>
            <tr align="center">
                ...
                <td align="center">{{ number_format($sum_target, 0) }}</td>
                <td align="center">{{ number_format($sum_sale_out, 0) }}</td>
                <td align="center"></td> <!-- ช่องว่างสำหรับ Achieve -->
                <td align="center">{{ number_format($sum_base_com, 0) }}</td>
                <td align="center">{{ number_format($sum_com_sale, 0) }}</td>
                <td align="center">{{ number_format($sum_extra_sale_out, 0) }}</td>
                <td align="center">{{ number_format($sum_extra_unit, 0) }}</td>
                <td align="center">{{ number_format($sum_extra_avg, 0) }}</td>
                <td align="center">{{ number_format($sum_other, 0) }}</td>
                <td align="center">{{ number_format($sum_total, 0) }}</td>
